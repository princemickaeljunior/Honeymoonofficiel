...
const I18N = {
  en: {
    ...
    guCompleteBody: "{count} universes discovered this round. You're already getting a feel for Honeymoon — and its creators.",
    guCompleteCta: "Discover Honeymoon Creator",
    guSignupCta: "Sign up free — join Honeymoon",
    ...
    guPathGameDesc: "10 mysteries to uncover, straight from the world of Honeymoon's creators. The wheel, the questions — different every time you play.",
    guPathPresTitle: "Roll the dice",
    ...
    guDiceCompleteBody: "{count} sides explored this round. You're already getting a feel for how Honeymoon — and its creators — really work.",
    ...
    subConfigHowP1: "1. Prepare your packs here — pick a type, a price, and upload the content.",
    subConfigHowP2: "2. Open a chat with a subscriber and tap the gold trophy icon above the gift button.",
    subConfigHowP3: "3. Pick the pack you want — it's sent instantly, already priced with their subscriber discount.",
    ...
    toolMatchWordsNote: "Two things in one here: get more out of chatting with creators on the site, or real advice on your flirt, dating & relationship life — pick a topic below.",
    toolMatchWordsWelcome: "Hi, I'm your Coach Honeymoon 💛 Pick a topic below to get started.",
    ...
    coachFrameSubtitle: "Two topics, two very different things",
    coachWelcomeIntro: "Hi, I'm your Coach Honeymoon 💛 Here's how I can help:",
    coachWelcomeBullet1: "How Honeymoon works, budget, offers & getting the most out of your favorite creators",
    coachWelcomeBullet2: "Real, general advice on flirt, dating & relationships — nothing to do with the site",
    coachWelcomeBullet3: "Two completely separate topics, so I can give you the right kind of advice",
    coachWelcomeBullet4: "Pick the one that fits what's on your mind right now",
    coachChooseThemePrompt: "Pick a topic:",
    coachThemeCreatorTitle: "Honeymoon & creators",
    coachThemeCreatorDesc: "How the site works, budget, offers, chatting with creators",
    coachThemeFlirtTitle: "Flirt, dating & relationship",
    ...
    coachBackBothTopics: "Back to both topics",
    ...
    chatNoSubPacksNote: "No subscriber packs in this conversation yet.",
    chatTabLibre: "Free chat",
    chatTabOrders: "Orders",
    chatTabAbonnement: "Subscription",
...
function applyStaticText(){
  ...
  document.getElementById('t-chat-tab-abonnement').textContent = t('chatTabAbonnement');
...
function renderMyProfile(slotId){
  ...
  body.innerHTML = `
    <div style="text-align:center;">
      ...
      <button class="sub-config-btn-primary" id="my-subscription-btn" style="max-width:280px;margin:10px auto 0;">${t('subConfigBtnLabel')}</button>
    </div>
...
const DESIRE_THEME_ICON = {
  flirt: goldGlowIcon('😘'),
  playful: goldGlowIcon('🎈'),
  talkative: goldGlowIcon('💬'),
  seduction: goldGlowIcon('🌹'),
  humor: goldGlowIcon('😂'),
  travel: goldGlowIcon('✈️'),
  music: goldGlowIcon('🎵'),
  restaurants: goldGlowIcon('🍽️'),
  movies: goldGlowIcon('🎬'),
  food: goldGlowIcon('🍓')
};
...
if(!document.getElementById('hm-coach-gold-style')){
  ...
  st.textContent = `
    ...
    .coach-back-both-btn{display:block;width:calc(100% - 37px);margin:14px 0 4px 37px;padding:12px;border-radius:14px;background:linear-gradient(90deg,var(--honey),var(--rose));color:#1c130a;font-weight:700;font-size:12px;text-align:center;border:none;cursor:pointer;}
    .coach-back-both-btn:active{opacity:.85;}
  `;
...
if(!document.getElementById('hm-shopbot-gold-style')){
  ...
  st2.textContent = `
    ...
    .guestuniverse-theme{--gu-c0:#ffb35c;--gu-c1:#ff8a3d;--gu-c2:#ff5a3d;--gu-c3:#e63946;--gu-c4:#ffd23d;}
    .gu-progress-wrap{margin:2px 0 14px;}
    .gu-progress-label{font-size:10.5px;color:var(--text-muted);margin-bottom:5px;}
    .gu-progress-bar{height:7px;border-radius:999px;background:var(--bg);border:1px solid var(--border);overflow:visible;display:flex;align-items:center;gap:6px;}
    /* ---- Barre : jaune -> orange -> rouge uniquement (jamais multicolore), avec
       un léger effet LED (halo qui pulse doucement) qui s'intensifie au fil du
       remplissage. ---- */
    .gu-progress-fill{height:100%;background:linear-gradient(90deg,#ffd23d,#ff8a3d,#ff4040);border-radius:999px;transition:width .5s ease, box-shadow .5s ease;flex:0 0 auto;box-shadow:0 0 6px 0 color-mix(in srgb, #ff8a3d 55%, transparent);animation:guBarLed 2.4s ease-in-out infinite;}
    @keyframes guBarLed{0%,100%{filter:brightness(1);}50%{filter:brightness(1.25);}}
    /* ---- Flamme en bout de barre : grise/éteinte tant que le jeu n'est pas terminé,
       s'allume (couleur + flicker LED) une fois les 15 univers découverts. ---- */
    ...
    .gu-dice-wrap{position:relative;display:flex;justify-content:center;align-items:center;padding:20px 0 14px;perspective:700px;-webkit-perspective:700px;}
    .gu-dice-cube{position:relative;width:96px;height:96px;transform-style:preserve-3d;-webkit-transform-style:preserve-3d;transition:transform 1.15s cubic-bezier(.22,.85,.25,1);}
    .gu-dice-cube.gu-dice-rolling{transition:transform 1.15s cubic-bezier(.22,.85,.25,1);}
    .gu-dice-face{position:absolute;inset:0;width:96px;height:96px;background:linear-gradient(135deg,#ffffff,#e9e3f8);border:3px solid #c4b5fd;border-radius:16px;display:flex;align-items:center;justify-content:center;backface-visibility:hidden;-webkit-backface-visibility:hidden;box-shadow:0 8px 14px rgba(0,0,0,.35);}
    ...
    .gu-video-placeholder{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;color:var(--text-muted);font-size:11px;font-weight:700;pointer-events:none;z-index:0;}
    /* ---- Diaporama de clôture : bulles de chat qui défilent, couleur de plus en
       plus chaude de la 1ère à la 20e diapo ---- */
    .gu-slideshow{margin-top:14px;}
    .gu-slideshow-viewport{overflow:hidden;border-radius:16px;border:1px solid var(--border);}
    .gu-slideshow-track{display:flex;transition:transform .5s cubic-bezier(.2,.8,.2,1);}
    .gu-slide{flex:0 0 100%;min-height:118px;display:flex;align-items:center;padding:4px;}
    .gu-slide-bubble{width:100%;border-radius:14px;padding:14px;text-align:left;animation:guSlideIn .45s ease;}
    .gu-slide-bubble .coach-bubble-text{background:none;border:none;padding:0;color:#fff;}
    .gu-slide-bubble .coach-mini-avatar{background:rgba(255,255,255,.18);color:#fff;}
    @keyframes guSlideIn{from{opacity:0;transform:scale(.96);}to{opacity:1;transform:scale(1);}}
    .gu-slideshow-dots{display:flex;flex-wrap:wrap;gap:4px;justify-content:center;margin-top:10px;}
    ...
    /* ---- Stades chromatiques progressifs, intensité renforcée : dès le départ (stade
       ...
       le fond s'assombrit et se réchauffe franchement — orange doux -> orange -> rouge
       corail -> rouge profond -> or, TOUJOURS dans les tons chauds (jamais de bleu/violet
       au passage), avec un glow pulsé de plus en plus fort. ---- */
    .guestuniverse-theme.gu-stage-0 .coach-frame{background:linear-gradient(160deg,color-mix(in srgb, var(--gu-c0) 8%, var(--bg-elev)),var(--bg-elev));border-color:color-mix(in srgb, var(--gu-c0) 16%, var(--border));}
    ...
    /* Le pointeur de roue reste violet à tous les stades — la roue ne suit plus
       la teinte du chat (voir guWheelColorPair()). */
    .guestuniverse-theme.gu-stage-1 .gu-wheel-pointer,
    .guestuniverse-theme.gu-stage-2 .gu-wheel-pointer,
    .guestuniverse-theme.gu-stage-3 .gu-wheel-pointer,
    .guestuniverse-theme.gu-stage-4 .gu-wheel-pointer{color:#c084fc;filter:drop-shadow(0 2px 6px color-mix(in srgb, #7c3aed 70%, transparent));}
...
const BOT_CONFIG = {
  creator: { categories: [
    { id: 'platform', icon: 'book', title: "How Honeymoon works" },
    { id: 'budget', icon: 'price', title: "Budget, tips & offers" },
    { id: 'engaging', icon: 'chat', title: "Chatting with creators" },
    { id: 'about', icon: 'eye', title: "Understanding creators" },
    { id: 'waiting', icon: 'device', title: "She hasn't replied yet" }
  ] }
};
/* Thème "flirt" : au lieu d'un tableau direct, on demande d'abord la situation du membre
   (en couple / célibataire en recherche / célibataire besoin de conseils), puis on affiche
   le tableau scopé à cette situation. Contenu 100% générique flirt/dating/relationship —
   aucune référence à une créatrice ou à la consommation du site. */
const FLIRT_STATUSES = {
  couple: { icon: 'heart', title: "I'm in a relationship", desc: "Communication, keeping the spark, staying close",
    categories: [
      { id: 'communicate', title: "Communicate better" },
      { id: 'spark', title: "Keep the spark alive" }
    ] },
  looking: { icon: 'sun', title: "I'm single & looking", desc: "Attraction, first dates, knowing if she's interested",
    categories: [
      { id: 'signals', title: "Does she like me?" },
      { id: 'attraction', title: "Create attraction" },
      { id: 'firstdate', title: "First date" }
    ] },
  advice: { icon: 'leaf', title: "I'm single & need advice", desc: "Confidence, moving on, general dating advice",
    categories: [
      { id: 'confidence', title: "Build confidence" },
      { id: 'moveon', title: "After a breakup" }
    ] }
};
...
/* Bouton "Retour aux 2 thèmes" (dégradé honey/rose, comme le badge CHAT), ajouté en bas
   de CHAQUE écran (tableau, réponse, choix de situation) — ramène direct à l'accueil. */
function coachBackBothHtml(){
  return `<button type="button" class="coach-back-both-btn" data-back-both="1">${escText(t('coachBackBothTopics'))}</button>`;
}
function coachWireBackBoth(container){
  const btn = container.querySelector('[data-back-both]');
  if(btn) btn.onclick = coachShowWelcomeUnified;
}

function coachChooseTheme(themeKey){
  coachState.theme = themeKey;
  coachState.status = null;
  if(themeKey === 'flirt'){ coachState.view = 'status'; coachShowFlirtStatusChoice(); }
  else { coachState.view = 'table'; coachRenderTable(); }
}

/* Thème "flirt" uniquement : demande d'abord la situation du membre avant d'afficher
   le tableau — pas de bouton en pas de puces ici, juste le choix direct (3 options). */
function coachShowFlirtStatusChoice(){
  const body = document.getElementById('coach-body');
  if(!body) return;
  const itemsHtml = Object.keys(FLIRT_STATUSES).map(key => {
    const s = FLIRT_STATUSES[key];
    return `<button type="button" class="coach-theme-btn" data-status="${key}"><span class="coach-theme-btn-icon">${AICON[s.icon]}</span><span><span class="coach-theme-btn-title">${escText(s.title)}</span><span class="coach-theme-btn-desc">${escText(s.desc)}</span></span></button>`;
  }).join('');
  body.innerHTML = `
    <div class="coach-bubble-row"><div class="coach-mini-avatar">🍯</div><div class="coach-bubble-text chat-bot show">${escText(t('coachStatusPrompt'))}</div></div>
    <div class="coach-theme-choice">${itemsHtml}</div>
    ${coachBackBothHtml()}
  `;
  body.querySelectorAll('[data-status]').forEach(btn => { btn.onclick = () => coachChooseStatus(btn.dataset.status); });
  coachWireBackBoth(body);
  body.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
}
function coachChooseStatus(statusKey){
  coachState.status = statusKey;
  coachState.view = 'table';
  coachRenderTable();
}

/* Étape 3/4 : tableau listant toutes les questions du thème (ou, pour "flirt", de la
   situation choisie), groupées par catégorie. La catégorie "waiting" (thème 1) n'a pas
   de liste de questions (c'est un mini-diagnostic à étapes) : une seule ligne cliquable. */
function coachRenderTable(){
  const body = document.getElementById('coach-body');
  if(!body) return;
  coachState.view = 'table';
  const cfg = coachState.theme === 'flirt' ? FLIRT_STATUSES[coachState.status] : BOT_CONFIG.creator;
  if(!cfg) return;
  const rowsHtml = cfg.categories.map(cat => {
    const catHead = `<div class="coach-table-cat">${cat.icon ? premiumTagIcon(AICON[cat.icon]) : ''} ${escText(cat.title)}</div>`;
    const starterIds = COACH_STARTERS[cat.id];
    if(!starterIds){
      return catHead + `<button type="button" class="coach-table-row" data-silence="1">${escText(cat.title)}</button>`;
    }
    return catHead + starterIds.map(id => `<button type="button" class="coach-table-row" data-node="${id}">${escText(COACH_NODES[id].q)}</button>`).join('');
  }).join('');
  const changeStatusBtn = coachState.theme === 'flirt'
    ? `<button type="button" class="coach-switch-theme" id="coach-switch-status">${COACH_ICON_BACK} ${escText(t('coachChangeStatus'))}</button>`
    : '';
  body.innerHTML = `
  ...
  const statusBtn = document.getElementById('coach-switch-status');
  if(statusBtn) statusBtn.onclick = coachShowFlirtStatusChoice;
  coachWireBackBoth(body);
}
...
/* ---- Mini-diagnostic à étapes pour "Elle n'a pas encore répondu" (thème 1 — angle
   service/consommation : message ou commande en attente, pas silence romantique) ----
   3 petites questions à choix, on construit un état, puis on compose une réponse
   sur-mesure en assemblant des blocs de texte selon les 3 réponses avant de proposer
   les mots-clés de suivi habituels et la même barre d'actions que pour une question normale. */
const COACH_SILENCE_STEPS = [
  { key: 'duration', q: "How long has it been since she last replied?", options: ['1 day', '3 days', '1 week', '+2 weeks'] },
  { key: 'type', q: "What are you waiting on?", options: ['A message', 'A custom order'] },
  { key: 'usual', q: "Does she usually reply to you quickly?", options: ['Yes', 'No', 'Not sure'] }
];
const COACH_SILENCE_DURATION_TXT = {
  '1 day': "A single day is nothing to worry about — creators manage a high volume of messages and orders every day.",
  '3 days': "A few days can happen during busy periods — it doesn't necessarily mean anything went wrong with your message or order.",
  '1 week': "A full week is worth a polite follow-up — a short, friendly check-in message is completely normal and appreciated.",
  '+2 weeks': "After two weeks, it's fair to send one clear follow-up, and consider reaching out to Honeymoon support if it's a paid order that hasn't been delivered."
};
const COACH_SILENCE_NORMAL_TXT = {
  'A message': "For a simple message, replies depend entirely on how many conversations she's juggling that day.",
  'A custom order': "Custom content takes real production time — filming, editing and reviewing — so orders naturally take longer than a quick chat reply."
};
const COACH_SILENCE_FIRST_TXT = {
  'Yes': "Since she's usually quick with you, this delay is likely just a busier-than-usual moment on her side.",
  'No': "If replies are typically slower with her, this isn't new behavior — it's just her normal pace.",
  'Not sure': "If you're not sure of her usual rhythm yet, give it a little more time before drawing any conclusions."
};
function renderCoachSilenceStep(stepNum, state){
  ...
  body.insertAdjacentHTML('beforeend', coachBackBothHtml());
  coachWireBackBoth(body);
  body.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
}
function coachShowSilenceDiagnosis(answers){
  ...
  const rawAnswer = [
    ...
    COACH_SILENCE_NORMAL_TXT[answers.type],
    COACH_SILENCE_FIRST_TXT[answers.usual]
  ].filter(Boolean).join(' ');
  ...
  setTimeout(() => {
    ...
    coachState.log.push({ theme: coachState.theme, q: "She hasn't replied yet", a: rawAnswer, ts: Date.now() });
    const chips = [
...
const COACH_STARTERS = {
  ...
  spark: ['spark_1','spark_2','spark_3','spark_4','spark_5','spark_6'],
  signals: ['sig_1','sig_2','sig_3','sig_4','sig_5','sig_6'],
  attraction: ['attr_1','attr_2','attr_3','attr_4','attr_5','attr_6'],
  firstdate: ['date_1','date_2','date_3','date_4','date_5','date_6'],
  confidence: ['conf_1','conf_2','conf_3','conf_4','conf_5','conf_6'],
  moveon: ['mv_1','mv_2','mv_3','mv_4','mv_5','mv_6']
};

/* ---- Contenu éditorial. Thème 1 (platform/budget/engaging/about) : 100% consommation
   du site — comment ça marche, budget, comment échanger avec une créatrice — général,
   jamais tourné en conseil de couple. Thème 2 (communicate/spark/signals/attraction/
   firstdate/confidence/moveon) : 100% flirt/dating/relationship réel, aucune mention
   du site ni d'une créatrice. Pas de chips de suivi sur ce nouveau contenu : la
   navigation se fait via le tableau et les boutons d'action. ---- */
const COACH_NODES = {
  // ======== THÈME 1 — Profiter du site (consommation) ========
  // -------- How Honeymoon works --------
  plat_1: { q: "How do I unlock a creator's paid content?", a: "Open her Tip Menu and pick what you want — photo, video or audio — then confirm the price. Once unlocked, it stays in your Collection so you can revisit it anytime." },
  plat_2: { q: "Is chatting with a creator free?", a: "Yes, messaging is free — you only pay for the content you choose to unlock through the Tip Menu or a custom request. There's no cost just to say hello." },
  plat_3: { q: "What's the difference between the Tip Menu and a custom request?", a: "The Tip Menu is ready-made content she's already priced and listed. A custom request is something you ask her to create specifically for you, priced by her at delivery." },
  ...
  bud_2: { q: "How do I know if an offer is worth the price?", a: "Check what's actually included — length, format, description — rather than just the number. A clear, detailed listing is usually a good sign of quality." },
  bud_3: { q: "Can I negotiate a price with a creator?", a: "Tip Menu prices are fixed by the creator, but for a custom request you can describe what you want and she'll quote a price back before you commit." },
  ...
  eng_2: { q: "How do I stand out among all the messages she gets?", a: "A real question about something she actually posted puts you in a different category than the generic messages she gets constantly." },
  eng_3: { q: "Is it okay to be playful or a bit flirty in chat?", a: "Light and playful is usually welcomed — just remember it's part of the experience she's offering, so stay respectful of her boundaries and pace." },
  eng_4: { q: "How often should I message a creator?", a: "There's no fixed rhythm — a message with something real to say lands better than checking in just to seem present." },
  eng_5: { q: "What makes creators reply faster?", a: "Clear, friendly messages that are easy to answer — and being a supporter who engages with her content — tend to get quicker replies." },
  eng_6: { q: "How do I build a good ongoing relationship as a supporter?", a: "Consistency and respect go a long way — show genuine interest in her content, tip when you can, and she'll remember you as a real supporter." },

  // -------- Understanding creators --------
  abt_1: { q: "Why aren't creators always available to chat?", a: "Most creators manage a large number of conversations and their own content production — treat her time as valuable, the same way you would anyone running a business." },
  abt_2: { q: "Why do prices vary between creators?", a: "Each creator sets her own prices based on her content, demand, and experience — there's no single standard rate across the platform." },
  abt_3: { q: "Do creators read every message they get?", a: "Most try to, but volume means replies can take time — a clear, easy-to-answer message helps yours stand out and get read faster." },
  abt_4: { q: "Is it normal that she chats with many members?", a: "Yes — that's simply how the platform works for her as a business. What matters is how she treats your specific conversation." },
  abt_5: { q: "How do creators decide what content to make?", a: "Many base new content on what their supporters request and respond well to — your feedback and requests genuinely help shape what she creates." },
  abt_6: { q: "Why is she sometimes slow to deliver a custom order?", a: "Custom content takes real time to film, edit and deliver — a short wait is normal, especially for detailed or personalized requests." },

  // -------- Mots-clés de suivi utilisés par le diagnostic "Elle n'a pas encore répondu" --------
  fu_silence: { q: "Should I ever just stop messaging and wait?", a: "If you've reached out reasonably and heard nothing yet, stepping back for a bit isn't a mistake — a reply usually comes once she's through her queue.",
    chips: [{ to: 'fu_patience' }] },
  fu_message: { q: "What should I send after a few quiet days?", a: "Something short and low-pressure — a light comment works better than asking why she went quiet. It gives her an easy way back into the conversation.",
    chips: [{ to: 'fu_silence' }] },
  fu_patience: { q: "How much patience is reasonable here?", a: "Enough to let a normal reply window pass without spiraling — a day or two of quiet from a busy creator rarely means anything.",
    chips: [{ to: 'fu_message' }] },

  // ======== THÈME 2 — Flirt, dating & relationship (aucun lien avec le site) ========
  // -------- In a relationship: Communicate better --------
  comm_1: { q: "How do I bring up something bothering me without starting a fight?", a: "Lead with how you feel rather than what she did wrong — 'I felt hurt when...' opens a conversation, while 'you always...' triggers defensiveness." },
  comm_2: { q: "She shuts down when we try to talk about problems, what do I do?", a: "Give her space to process before pushing for a full conversation — some people need a little time before they can talk calmly." },
  comm_3: { q: "How much should we share about our day with each other?", a: "There's no fixed amount — what matters is that both of you feel heard when you do share, not the volume of details." },
  comm_4: { q: "We keep having the same argument over and over", a: "Recurring arguments usually point to an unmet need underneath, not the surface topic — try naming what you actually need instead of refighting the same point." },
  comm_5: { q: "How do I bring up an insecurity I have in the relationship?", a: "Choose a calm moment, not mid-argument, and frame it as your own feeling rather than an accusation — it makes her far more likely to actually hear you." },
  comm_6: { q: "Is it normal to disagree this often?", a: "Yes — disagreeing isn't the problem, it's how you handle it together that determines whether it strengthens or wears down the relationship." },

  // -------- In a relationship: Keep the spark alive --------
  spark_1: { q: "How do we keep things exciting after being together a while?", a: "Small novelty matters more than big gestures — a new activity together, a spontaneous plan, or simply being fully present breaks the routine." },
  spark_2: { q: "I feel like the flirt disappeared from our relationship", a: "Flirting often fades because it stops being intentional — a playful text or a genuine compliment during the day brings it back faster than people expect." },
  spark_3: { q: "How do I plan a date that doesn't feel repetitive?", a: "Try something neither of you has done before — new activities create shared memories that a repeated dinner-and-movie routine rarely does." },
  spark_4: { q: "Should I still make an effort with how I look for her?", a: "Yes, in the same way you'd want her to keep making an effort for you — small consistent effort matters more than the occasional big one." },
  spark_5: { q: "How do we stay close when life gets busy?", a: "Protect small daily moments — a real conversation over coffee, a message that isn't logistics — busy periods don't have to mean disconnected ones." },
  spark_6: { q: "Is it normal to feel like roommates sometimes?", a: "It happens in most long relationships during busy stretches — noticing it is actually the first step to intentionally bringing the spark back." },

  // -------- Single & looking: Does she like me? --------
  sig_1: { q: "How do I know if she's really into me?", a: "Look at what she does without being asked — replying with real interest, remembering details, initiating sometimes. Consistency without being prompted is the real signal." },
  sig_2: { q: "She's friendly with everyone, is it real with me?", a: "General friendliness doesn't tell you much — what matters is whether she treats you differently one-on-one, not how she is in a group." },
  sig_3: { q: "She's slow to reply, does that mean she's not interested?", a: "Slow replies usually mean she's busy, not disinterested — focus on the quality of what she says when she does answer." },
  sig_4: { q: "Does she remember what I tell her?", a: "If she brings up something you mentioned before without being reminded, that's a genuine sign she's actually paying attention to you." },
  sig_5: { q: "She uses pet names with me, does that mean something?", a: "It's a nice sign, but pair it with other signals — memory, initiation, consistency — before reading too much into a single habit." },
  sig_6: { q: "How do I ask her clearly if she's interested?", a: "Keep it light and low-pressure — asking what she enjoys about your conversations works better than demanding she define things outright." },

  // -------- Single & looking: Create attraction --------
  attr_1: { q: "How do I stand out from everyone else messaging her?", a: "A real question about something she actually said or did instantly puts you in a different category than generic compliments." },
  attr_2: { q: "Does playing hard to get actually work?", a: "A little unpredictability can be healthy, but games built on ignoring her read as immaturity — real attraction comes from having your own life and energy." },
  attr_3: { q: "How important is humor?", a: "Very — a genuine, light joke beats a rehearsed line almost every time. If she's playful, she wants someone who can match that energy." },
  attr_4: { q: "Should I compliment her looks or something else?", a: "Looks-only compliments blend into everything else she hears — noticing her humor or a specific detail shows you're actually paying attention." },
  attr_5: { q: "Is confidence really that important?", a: "Yes, but it means being relaxed and not needing constant approval — not being loud. Calm, secure energy reads as attractive over time." },
  attr_6: { q: "How do I create a bit of tension or interest?", a: "A touch of mystery works better than oversharing everything at once — leave a little for her to ask about." },

  // -------- Single & looking: First date --------
  date_1: { q: "Where should I take her on a first date?", a: "Somewhere you can actually talk — a busy bar or loud venue makes connecting harder than a relaxed coffee, walk, or casual dinner." },
  date_2: { q: "How do I avoid awkward silences?", a: "A few silences are completely normal and not a bad sign — have one or two open questions in your back pocket, but don't force constant talking." },
  date_3: { q: "Should I plan the whole date or keep it casual?", a: "Have a loose plan so you're not scrambling, but stay flexible — the best dates often shift naturally once the conversation gets going." },
  date_4: { q: "How do I know if the date is going well?", a: "Watch for her leaning in, asking questions back, and losing track of time — those matter far more than any single thing either of you says." },
  date_5: { q: "Who should pay on a first date?", a: "There's no universal rule — offering is always a good look, and how she responds usually tells you what she's comfortable with." },
  date_6: { q: "Should I go for a second date fast or wait?", a: "If it went well, there's no reason to wait — a message the same or next day saying you enjoyed it works better than playing it cool." },

  // -------- Single & need advice: Build confidence --------
  conf_1: { q: "How do I stop overthinking every message I send?", a: "Give yourself a rule — write it, read it once, send it. Overthinking rarely improves a message, it just delays it." },
  conf_2: { q: "I feel like I'm not interesting enough", a: "Interesting isn't a fixed trait — it's showing genuine curiosity and energy in the moment, which anyone can bring regardless of how their life looks on paper." },
  conf_3: { q: "How do I get over the fear of rejection?", a: "Reframe it as information, not failure — a 'no' just means that specific match wasn't right, not a verdict on you." },
  conf_4: { q: "How do I build confidence before a date?", a: "Prepare a little — know where you're going, have a couple of things to talk about — so nerves come from excitement, not unpreparedness." },
  conf_5: { q: "I compare myself to other guys a lot", a: "Comparison rarely reflects reality — you're seeing someone else's highlight reel, not the full picture, so it's a poor benchmark for your own worth." },
  conf_6: { q: "How do I stop seeming needy or too eager?", a: "Keep your own life full and interesting — genuine excitement is attractive, but making someone your only source of validation reads as pressure." },

  // -------- Single & need advice: After a breakup --------
  mv_1: { q: "How long should I wait before dating again?", a: "There's no universal number — the real marker is whether you're looking for genuine connection or just distraction from the pain." },
  mv_2: { q: "I keep comparing new people to my ex", a: "Very normal early on — just notice when it happens so you can catch yourself. Someone new deserves to be seen for who they are." },
  mv_3: { q: "How do I stop checking her social media?", a: "Muting or unfollowing for a while isn't petty, it's practical — it removes the temptation while you actually heal." },
  mv_4: { q: "Is it normal to still have feelings months later?", a: "Yes, timelines vary a lot — what matters is whether those feelings are shrinking over time, not whether they've vanished completely." },
  mv_5: { q: "How do I know I'm ready to date again?", a: "You're generally ready when you can think about your ex without it derailing your whole day, not when the feelings have disappeared entirely." },
...
/* ==================================================================================
   ---------------- "Univers Invité" (Honeymoon) — v3 ----------------
   Roue SVG (20 secteurs, icônes 100% traits) menant à un mini quiz de personnalité
   (5 questions × 3 choix) par thème — 10 thèmes "quel type de créatrice t'attire"
   et 10 thèmes qui, sans jamais le nommer pendant le jeu, font émerger un besoin
   affectif/relationnel réel (solitude, besoin d'être compris, confiance en
   approche...), validé avec empathie. À la toute fin (20/20), un écran teaser
   relie ces besoins au futur produit "Honeymoon Coach" (accompagnement
   célibataires/couples) — avec un emplacement <video> prêt à recevoir le fichier
   final (je ne peux pas générer de vidéo moi-même).
   Réutilise le moteur visuel du Coach Honeymoon (.coach-frame, .coach-bubble-row)
   via .guestuniverse-theme. Progression mémorisée dans Firestore (guestJourneys),
   clé = identifiant visiteur anonyme conservé en localStorage (pas de compte requis).
   ================================================================================== */
...
const GU_RATING_ICONS = {
  loved: '<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor" stroke="none"><path d="M20.8 4.6a5.5 5.5 0 0 0-7.8 0L12 5.6l-1-1a5.5 5.5 0 0 0-7.8 7.8l1 1L12 21l7.8-7.8 1-1a5.5 5.5 0 0 0 0-7.6z"/></svg>',
  good: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M8.5 14.2c1 1.1 2.2 1.7 3.5 1.7s2.5-.6 3.5-1.7"/><path d="M8.7 9.5h.01M15.3 9.5h.01"/></svg>',
  meh: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M8.5 15h7"/><path d="M8.7 9.5h.01M15.3 9.5h.01"/></svg>',
  no: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><path d="M8.5 15.8c1-1.1 2.2-1.7 3.5-1.7s2.5.6 3.5 1.7"/><path d="M8.7 9.5h.01M15.3 9.5h.01"/></svg>'
};

/* Options d'onboarding : comment le visiteur a découvert Honeymoon. */
const GU_DISCOVERY_OPTIONS = ['A friend told me about it', 'Social media', 'Just came across it', 'I already knew about it'];

/* 20 thèmes — 10 "type" (quel genre de créatrice t'attire) + 10 "need" (un besoin
   affectif/relationnel réel, validé avec empathie, jamais nommé comme un produit
   pendant le jeu). Chaque thème = 5 mini-questions à 3 choix, façon test de
   personnalité — dans le même esprit que le Coach Honeymoon. Configurable :
   ajouter/retirer/modifier une entrée suffit. */
/* 10 thèmes (5 "type" côté créatrice + 5 "need" côté visiteur) — chaque énigme
   met en scène soit une créatrice de la plateforme, soit le visiteur lui-même,
   toujours dans l'esprit du Coach Honeymoon (jamais nommé comme un produit
   pendant le jeu, juste préparé). 5 mini-questions à 3 choix par thème,
   écrites dans un registre volontairement émotionnel (le ressenti avant le
   fait), pas des questions froides/techniques. */
const GUEST_THEMES = [
  { id:'elegant', track:'type', icon:'gem', title:'Elegant & refined',
    short:'Elegant',    questions:[
      { q:'She walks in and the whole room shifts — what gets you first?', opts:['That smile','The way she carries herself','Something you can\'t quite name'] },
      ...
      { q:'What would make her feel completely unforgettable to you?', opts:['Her grace under pressure','The way she listens','A single perfect detail'] },
      { q:'If she never raised her voice, what would that tell you?', opts:['She doesn\'t need to','She\'s always in control','She respects herself too much to'] },
      { q:'What\'s the real difference between elegant and just polished?', opts:['Substance underneath','Ease, not effort','Knowing when to stay quiet'] }
    ],
  ...
  { id:'playful', track:'type', icon:'sparkle', title:'Playful & fun',
    short:'Playful',    questions:[
      { q:'What makes your chest actually loosen up when you\'re talking to her?', opts:['A sharp, witty line','Warm, goofy energy','A little teasing back and forth'] },
      ...
      { q:'What\'s the fastest way she could win you over completely?', opts:['A perfectly timed sarcastic reply','Making fun of herself first','Turning a boring moment into a joke'] },
      { q:'What kind of playful actually keeps your attention long term?', opts:['Quick wit','Warm teasing','Unexpected silliness'] },
      { q:'If she surprised you with something completely random, how would you react?', opts:['Laugh out loud','Play along instantly','Ask a hundred questions'] }
    ],
  ...
  { id:'confident', track:'type', icon:'crown', title:'Bold & confident',
    short:'Confident',    questions:[
      { q:'She says exactly what she means, no hedging — what does that stir in you?', opts:['Respect','Attraction','A little intimidation, honestly'] },
      ...
      { q:'What\'s the boldest thing she could say in a first message?', opts:['Exactly what she wants','A direct compliment','Nothing — just showing up is enough'] },
      { q:'How do you feel around someone who never apologizes for taking up space?', opts:['Inspired','A little challenged','Completely drawn in'] },
      { q:'What would make her confidence feel real, not performed?', opts:['She\'s consistent about it','She\'s calm, not loud','She\'s confident even when it\'s hard'] }
    ],
  ...
  { id:'sweet', track:'type', icon:'heart', title:'Sweet & gentle',
    short:'Sweet',    questions:[
      { q:'She replies and something in you just... relaxes. What did she say?', opts:['Something warm','Something caring','Something that made you feel safe'] },
      ...
      { q:'What\'s the smallest thing she could do that would stay with you for days?', opts:['A thoughtful check-in','Remembering something small','Just being patient with you'] },
      { q:'What does "safe" actually feel like, with someone new?', opts:['No judgment, ever','Being allowed to go slow','Just being taken seriously'] },
      { q:'If she noticed you were having a hard day without you saying it, how would that land?', opts:['Like relief','Like being truly seen','Like something rare'] }
    ],
  ...
  { id:'mysterious', track:'type', icon:'moonstar', title:'Mysterious & intense',
    short:'Mysterious',    questions:[
      { q:'She stays a little out of reach — does that pull you in, or push you away?', opts:['Pulls me in, hard','A little of both','Honestly, pushes me away'] },
      ...
      { q:'What\'s more attractive — someone easy to read, or someone worth figuring out?', opts:['Easy to read, honestly','Worth figuring out','Depends on the day'] },
      { q:'What would you do if she gave you one honest answer, unprompted?', opts:['Hold onto it','Ask for more','Just sit with it quietly'] },
      { q:'Where\'s the line between mysterious and just distant?', opts:['Whether she lets you in eventually','How it makes you feel','There isn\'t always one'] }
    ],
  ...
  { id:'loneliness', track:'need', icon:'users', title:'More than scrolling',
    short:'Not alone',    questions:[
      { q:'Be honest — what does the silence in your evenings actually feel like?', opts:['Peaceful, mostly','Heavier than I let on','Like something\'s missing'] },
      ...
      { q:'What does a "good" evening actually look like for you right now?', opts:['Someone to talk to, even briefly','Just not scrolling alone','A real conversation, not small talk'] },
      { q:'What\'s the hardest part about admitting you feel lonely?', opts:['Saying it out loud','Not knowing what to do about it','Worrying it makes me seem needy'] },
      { q:'If someone reached out to you first, tonight, what would that mean?', opts:['Everything, honestly','A relief I didn\'t expect','Proof I\'m not invisible'] }
    ],
  ...
  { id:'understood', track:'need', icon:'headphones', title:'Being heard',
    short:'Being heard',    questions:[
      { q:'How does it feel when someone finally gets you, right away?', opts:['Like a weight lifting','Rare, and precious','Almost unfamiliar'] },
      ...
      { q:'What\'s the difference between someone listening and someone actually hearing you?', opts:['They remember it later','They ask a real follow-up','They don\'t try to fix it'] },
      { q:'What do you usually do instead of saying what you actually mean?', opts:['Change the subject','Make a joke of it','Just stay quiet'] },
      { q:'If someone got it right on the first try, how would that feel?', opts:['Shocking, honestly','A huge relief','Like I could finally exhale'] }
    ],
  ...
  { id:'dating_confidence', track:'need', icon:'warning', title:'Feeling sure of yourself',
    short:'Feeling sure',    questions:[
      { q:'Your thumb hovers over "send" — what\'s actually racing through you?', opts:['Excitement','Straight-up fear','A mix I can\'t untangle'] },
      ...
      { q:'What\'s the story you tell yourself right before you back out?', opts:['"They\'ll say no anyway"','"I\'ll mess it up"','"It\'s not worth the risk"'] },
      { q:'What would it take to actually believe you\'re worth the effort?', opts:['One person proving it','Time','Deciding it myself, first'] },
      { q:'If you stopped waiting for permission to want something, what would change?', opts:['I\'d actually try','I\'d stop overthinking every move','I\'d finally feel like myself'] }
    ],
  ...
  { id:'after_breakup', track:'need', icon:'lock', title:'Starting somewhere new',
    short:'Starting fresh',    questions:[
      { q:'Underneath it all, where are you really, after the last one?', opts:['Still aching a little','Mostly okay, but cautious','Genuinely ready'] },
      ...
      { q:'What\'s the one thing you swore you wouldn\'t do again?', opts:['Rush it','Ignore red flags','Lose myself trying to keep it'] },
      { q:'What would "moving on" actually look like for you, honestly?', opts:['Feeling lighter, not numb','Being open again, carefully','Just not flinching at reminders anymore'] },
      { q:'What\'s still hard to say out loud about the last one?', opts:['That it still stings a little','That I gave too much','That I\'m scared to do it again'] }
    ],
  ...
  { id:'real_connection', track:'need', icon:'trophy', title:'Something that actually lasts',
    short:'Something real',    questions:[
      { q:'Deep down, are you here for something light, or something that could actually matter?', opts:['Light, for now','Something real','Honestly not sure yet'] },
      ...
      { q:'What\'s the one thing that separates "real" from just "easy"?', opts:['It takes effort, and still feels good','Honesty, even when it\'s hard','It doesn\'t disappear when things get quiet'] },
      { q:'What would you give up to have something that actually lasts?', opts:['The chase','Being guarded','Some of my independence, a little'] },
      { q:'If you found it, how would you even know?', opts:['I\'d stop checking my phone so much','I\'d feel calm, not anxious','I\'d just know — no doubt'] }
    ],
...
const GU_PHRASE_BANKS = {
  ...
  askDiscovery: [
    "One more thing, {name} — how did you first come across Honeymoon?",
    "Quick one before we start, {name} — how did you first hear about Honeymoon?",
    "{name}, one last thing first — what brought you to Honeymoon?",
    "Before we dive in, {name} — how did Honeymoon first cross your path?",
    "One more question, {name} — what's the story of how you found Honeymoon?"
  ],
  askPath: [
    "One last thing before we start, {name} — how do you want to explore?",
    "Almost there, {name} — how would you like to play?",
    "{name}, last step — pick how you want to dive in.",
    "One more choice, {name} — which way do you want to explore Honeymoon?",
    "Before we jump in, {name} — how do you want to discover things?"
  ]
};
let guLastPhrase = {};
function guVary(key){
...
/* ================================================================
   ...
   Même concept que la roue (mystère -> questions -> révélation), mais
   6 faces au lieu de 20 cases, et un thème unique : comment Honeymoon
   fonctionne réellement — toujours amené par Coach Honeymoon, qui est
   justement la personne qui explique aussi le site aux visiteurs. */
const SITE_DICE_THEMES = [
  { id:'d_creators', icon:'gem', title:'How creators get chosen',
    questions:[
      ...
      { q:'What would make a creator\'s profile stand out to you immediately?', opts:['A clear, honest bio','Real conversation samples','Just her energy'] },
      { q:'How much does consistency matter compared to a strong first impression?', opts:['More, long term','About the same','Less — first impression wins'] },
      { q:'What\'s the one thing that would make you stick with a creator for months?', opts:['She keeps showing up as herself','She remembers you','She keeps things interesting'] }
    ],
  ...
  { id:'d_chat', icon:'chat', title:'How the chat really works',
    questions:[
      ...
      { q:'What makes a conversation feel worth continuing past the first few messages?', opts:['Curiosity on both sides','Ease, not effort','A little unpredictability'] },
      { q:'Would you rather a slower, thoughtful reply or a fast, casual one?', opts:['Slower and thoughtful','Fast and casual','Depends on my mood'] },
      { q:'What\'s the sign a conversation is actually going somewhere?', opts:['Questions, not just answers','She remembers past messages','It doesn\'t feel like work'] }
    ],
  ...
  { id:'d_subs', icon:'gift', title:'Subscriptions & content packs',
    questions:[
      ...
      { q:'What would make you feel confident hitting subscribe?', opts:['Clear expectations','Seeing real content first','Trusting the creator already'] },
      { q:'How do you decide if a pack is actually worth the price?', opts:['Compare it to what\'s included','Go with gut feeling','Ask before buying'] },
      { q:'What would make you loyal to one creator instead of browsing around?', opts:['She feels consistent','She feels personal','She keeps surprising me'] }
    ],
  ...
  { id:'d_privacy', icon:'lock', title:'Privacy & safety',
    questions:[
      ...
      { q:'What would make you trust a platform with your information?', opts:['Clear policies','Seeing others use it safely','Just time'] },
      { q:'How important is staying anonymous to you here?', opts:['Very — it\'s a dealbreaker','Somewhat, but flexible','Not really a concern'] },
      { q:'What would make you feel in control of your own experience here?', opts:['Easy settings to manage','Knowing exactly what\'s shared','Being able to walk away anytime'] }
    ],
  ...
  { id:'d_coach', icon:'moonstar', title:'What Coach Honeymoon actually does',
    questions:[
      ...
      { q:'What\'s the biggest thing holding most people back from asking for help?', opts:['Pride','Not knowing where to start','Thinking they should already know'] },
      { q:'Would advice actually change how you approach things, or just confirm what you know?', opts:['Change it, honestly','Confirm it, mostly','A bit of both'] },
      { q:'What would it mean to finally have a clear plan instead of guessing?', opts:['A relief','More confidence','Actually taking action'] }
    ],
...
async function guLoadJourney(){
  try{
    ...
    if(doc.exists){
      ...
      guState.discoverySource = d.discoverySource || null;
...
function guRenderRoulette(){
  ...
  if(guState.path === 'dice'){ guRenderDiceRoulette(body); return; }
...
/* Choix explicite, juste après nom + source : partie complète à la roue (15
   mystères côté créatrices/visiteurs) ou partie courte au dé à 6 faces
   (fonctionnement du site, tournée elle aussi vers Coach Honeymoon). Les deux
   mènent au même mécanisme de mystère/coach à la fin. */
function guRenderPathChoice(body){
  body.innerHTML = `
    <div class="coach-bubble-row"><div class="coach-mini-avatar">🍯</div><div class="coach-bubble-text chat-bot show">${escText(guVary('askPath').replace('{name}', escText(guState.firstName)))}</div></div>
    <button type="button" class="gu-path-card" id="gu-path-game">
      <div class="gu-path-card-icon">${guIcon('compass',22)}</div>
      <div class="gu-path-card-title">${escText(t('guPathGameTitle'))}</div>
      <div class="gu-path-card-desc">${escText(t('guPathGameDesc'))}</div>
    </button>
    <button type="button" class="gu-path-card" id="gu-path-dice">
...
function guRenderDiscoveryStep(body){
  body.innerHTML = `
    <div class="coach-bubble-row"><div class="coach-mini-avatar">🍯</div><div class="coach-bubble-text chat-bot show">${escText(guVary('askDiscovery').replace('{name}', escText(guState.firstName)))}</div></div>
  ...
  body.querySelectorAll('[data-disc]').forEach(btn => {
    btn.onclick = async () => {
      guState.discoverySource = GU_DISCOVERY_OPTIONS[btn.dataset.disc];
      await guSaveJourney({ discoverySource: guState.discoverySource });
      guState.view = 'wheel';
      guRenderRoulette();
    };
  });
...
/* Paire de couleurs des secteurs : toujours sur des tons violets (demande explicite),
   la roue ne suit plus la progression chromatique du reste du chat. */
...
function guBuildWheelSvg(restingDeg){
  ...
  themesOrdered.forEach((th, i) => {
    ...
    // Cases numérotées (1-15) plutôt que le titre en toutes lettres : plus de
    // risque de chevauchement entre cases voisines, quel que soit l'angle. Le
    // vrai titre + son icône n'apparaissent qu'une fois la question posée
    // (guRenderQuizStep) — c'est là qu'il y a la place pour être lisible.
    const tx = cx + (R * 0.68) * Math.cos(mid * Math.PI / 180), ty = cy + (R * 0.68) * Math.sin(mid * Math.PI / 180);
    let rot = mid + 90;
    if(rot > 90 && rot < 270) rot += 180;
    if(th.type === 'mystery'){
      labels += `<text x="${tx.toFixed(2)}" y="${ty.toFixed(2)}" transform="rotate(${rot.toFixed(2)},${tx.toFixed(2)},${ty.toFixed(2)})" text-anchor="middle" dominant-baseline="middle" font-size="16" font-weight="800" fill="#fff" font-family="inherit">?</text>`;
    }else{
      const label = guWheelLabel(th, themeIdx);
      themeIdx++;
      labels += `<text x="${tx.toFixed(2)}" y="${ty.toFixed(2)}" transform="rotate(${rot.toFixed(2)},${tx.toFixed(2)},${ty.toFixed(2)})" text-anchor="middle" dominant-baseline="middle" font-size="14" font-weight="800" fill="#fff" font-family="inherit" style="paint-order:stroke;stroke:#2e1065;stroke-width:2px;stroke-linejoin:round;">${label}</text>`;
    }
  });
  return `<svg id="gu-wheel-svg" viewBox="0 0 280 280" width="230" height="230">
    <circle cx="${cx}" cy="${cy}" r="${R + 6}" fill="#2e1065"/>
    <g>${sectors}</g>
    <g>${labels}</g>
...
function guRenderMysteryQuiz(body){
  ...
  body.querySelectorAll('[data-mystery-pick]').forEach(btn => {
    btn.onclick = async () => {
      const isCorrect = Number(btn.dataset.mysteryPick) === riddle.correct;
...
function guRenderMysteryBreak(body){
  ...
  body.querySelectorAll('[data-mystery-pick]').forEach(btn => {
    btn.onclick = async () => {
      ...
      guRenderMysteryUnlock(body, riddle, undefined, isCorrect);
    };
  });
}
...
function guRenderMysteryUnlock(body, riddle, onContinue, wasCorrect){
...
function guRenderPresQuizStep(body){
  ...
  if(item.kind === 'mystery'){
    ...
    body.querySelectorAll('[data-pqpick]').forEach(btn => {
      btn.onclick = async () => {
        const isCorrect = Number(btn.dataset.pqpick) === riddle.correct;
        guState.mysteryUnlocked.push(riddle.id);
        guState.totalQuestionsAnswered = (guState.totalQuestionsAnswered || 0) + 1;
      guState.roundQuestionsAnswered = (guState.roundQuestionsAnswered || 0) + 1;
        guApplyStage(document.getElementById('gu-theme-root'));
        await guSaveJourney({ mysteryUnlocked: guState.mysteryUnlocked, totalQuestionsAnswered: guState.totalQuestionsAnswered });
        guRenderMysteryUnlock(body, riddle, () => { guState.presQuizStep++; guRenderPresQuizStep(body); }, isCorrect);
      };
    });
    return;
  }
...
/* Écran final (20/20) : le rideau tombe — relie ce qui vient d'émerger au futur
   produit "Honeymoon Coach". Emplacement <video> prêt à recevoir le fichier final
...
let guSlideshowIdx = 0;
function guStopSlideshow(){
  if(guSlideshowInterval){ clearInterval(guSlideshowInterval); guSlideshowInterval = null; }
}
function guGotoSlide(idx, slides){
  const track = document.getElementById('gu-slideshow-track');
  const dots = document.getElementById('gu-slideshow-dots');
  if(!track) return;
  guSlideshowIdx = (idx + slides.length) % slides.length;
  track.style.transform = `translateX(-${guSlideshowIdx * 100}%)`;
  if(dots){
    dots.querySelectorAll('.gu-slide-dot').forEach((d,i) => d.classList.toggle('active', i === guSlideshowIdx));
  }
}
  ...
  return `
    <div class="gu-slideshow" id="gu-slideshow">
      <div class="gu-slideshow-viewport">
        <div class="gu-slideshow-track" id="gu-slideshow-track">${slidesHtml}</div>
      </div>
      <div class="gu-slideshow-dots" id="gu-slideshow-dots">${dotsHtml}</div>
    </div>
  `;
}
function guWireSlideshow(){
  const slides = guSlideshowOrder();
  guSlideshowIdx = 0;
  guStopSlideshow();
  const el = document.getElementById('gu-slideshow');
  if(!el) return;
  el.querySelectorAll('[data-goto]').forEach(dot => {
    dot.onclick = () => { guGotoSlide(Number(dot.dataset.goto), slides); guStopSlideshow(); guSlideshowInterval = setInterval(() => guGotoSlide(guSlideshowIdx + 1, slides), 3800); };
  });
  el.onclick = (e) => { if(e.target.closest('[data-goto]')) return; guGotoSlide(guSlideshowIdx + 1, slides); };
...
function guOpenRealCoach(){
  ...
  document.getElementById('gu-coach-back-to-game').onclick = () => {
    if(guState.path === 'dice'){ guState.diceView = 'dice'; } else { guState.view = 'wheel'; }
    guRenderRoulette();
  };
}
/* Accès direct à la vraie page d'inscription (le même formulaire que le reste
   du site) depuis les écrans de fin de Guest Universe — quitte le mini-jeu et
...
function guRenderComplete(body){
  ...
  body.innerHTML = `
    <div class="gu-complete-card">
      ...
      <div class="gu-theme-card-desc" style="margin-top:6px;">${escText(t('guCompleteBody').replace('{count}', guState.discovered.length))}</div>
      ${guRenderSlideshow()}
...
function guRenderDiceMysteryQuiz(body){
  ...
  body.querySelectorAll('[data-dmpick]').forEach(btn => {
    btn.onclick = async () => {
      const isCorrect = Number(btn.dataset.dmpick) === riddle.correct;
      if(!guState.mysteryUnlocked.includes(riddle.id)) guState.mysteryUnlocked.push(riddle.id);
      guState.currentMystery = null;
      guState.totalQuestionsAnswered = (guState.totalQuestionsAnswered || 0) + 1;
      guState.roundQuestionsAnswered = (guState.roundQuestionsAnswered || 0) + 1;
      ...
      guApplyStage(document.getElementById('gu-theme-root'));
      await guSaveJourney({ mysteryUnlocked: guState.mysteryUnlocked, totalQuestionsAnswered: guState.totalQuestionsAnswered });
      guRenderMysteryUnlock(body, riddle, () => { guState.diceView = 'dice'; guRenderRoulette(); }, isCorrect);
    };
  });
}
function guRenderDiceDrawn(body){
  ...
  body.innerHTML = `
    ...
    <div class="gu-choice-row">
      <button type="button" class="gu-choice-btn primary" id="gu-dice-accept">${guIcon('check',14)} ${escText(t('guAccept'))}</button>
    </div>
  `;
...
function guRenderDiceQuizStep(body){
  const theme = SITE_DICE_THEMES.find(th => th.id === guState.dicePendingId);
  if(!theme){ guState.diceView = 'dice'; guRenderRoulette(); return; }
  ...
  body.querySelectorAll('[data-dpick]').forEach(btn => {
    btn.onclick = () => {
      guState.diceQuizPicks.push(Number(btn.dataset.dpick));
      guState.totalQuestionsAnswered = (guState.totalQuestionsAnswered || 0) + 1;
      guState.roundQuestionsAnswered = (guState.roundQuestionsAnswered || 0) + 1;
      guApplyStage(document.getElementById('gu-theme-root'));
...
function guRenderDiceReveal(body){
  ...
  body.innerHTML = `
    <div class="gu-experience">
      ...
      <div class="gu-experience-title">${escText(theme.title)}</div>
      <div class="gu-experience-text">${escText(theme.reveal)}</div>
      <div class="gu-experience-tip">${guIcon('sparkle',14)} ${escText(theme.tip)}</div>
    </div>
    <button type="button" class="gu-spin-btn" id="gu-dice-continue" style="margin-top:14px;">${escText(t('guContinue'))}</button>
  `;
  document.getElementById('gu-dice-continue').onclick = async () => {
    if(!guState.diceDiscovered.includes(theme.id)){
...
function guRenderDiceComplete(body){
  ...
  body.innerHTML = `
    <div class="gu-complete-card">
      ...
      <div class="gu-theme-card-desc" style="margin-top:6px;">${escText(t('guDiceCompleteBody').replace('{count}', guState.diceDiscovered.length))}</div>
      ${recapHtml}
      <div class="gu-theme-card-desc" style="margin-top:10px;">${escText(t('guCoachPitch'))}</div>
      <div class="gu-theme-card-desc" style="margin-top:10px;">${escText(t('guBonusMention'))}</div>
      ${showWheelCta
...
function guRenderSessionPopup(body){
  body.innerHTML = `
    <div class="gu-session-popup">
      <div class="gu-experience-icon">🍯</div>
    ...
    body.innerHTML = `
      ...
      <div class="tabs-slide-row">
        <div class="member-tabs">
          ...
          <button type="button" class="member-tab" id="member-tab-tools"><span class="coach-gold-led">🍯 ${t('memberTabTools')}</span></button>
        </div>
      </div>
...
let chatActiveTab = 'libre'; // 'libre' (discussion libre), 'commandes' (demandes de commande) ou 'abonnement' (packs abonnés)
...
function renderChatMessages(ctx, convRef, unreadField){
  ...
  const docs = allDocs.filter(d => {
    const dd = d.data();
    if(chatActiveTab === 'abonnement') return !!dd.customOrderRequest && dd.orderKind === 'subPack';
    if(chatActiveTab === 'commandes') return !!dd.customOrderRequest && dd.orderKind !== 'subPack';
    return !dd.customOrderRequest;
  });
  if(docs.length === 0){
    const emptyMsg = chatActiveTab === 'commandes' ? t('chatNoOrdersNote') : chatActiveTab === 'abonnement' ? t('chatNoSubPacksNote') : t('chatEmptyNote');
    box.innerHTML = `<span class="chat-empty-note">${emptyMsg}</span>`;
    return;
  }
...
function updateChatTabBadges(){
  const ordersCount = chatLastSnapDocs.filter(d => d.data().customOrderRequest && d.data().orderKind !== 'subPack' && d.data().customOrderStatus !== 'delivered').length;
  ...
  const subPackCount = chatLastSnapDocs.filter(d => d.data().orderKind === 'subPack').length;
  const subBadge = document.getElementById('chat-tab-abonnement-badge');
  if(subBadge){
    if(subPackCount > 0){ subBadge.style.display = 'inline-flex'; subBadge.textContent = subPackCount > 99 ? '99+' : String(subPackCount); }
...
if(!document.getElementById('hm-subscribe-style')){
  ...
  st2.textContent = `
    ...
    /* ---- Page de configuration de l'offre (créatrice) — habillage premium ---- */
    .sub-config-btn-primary{display:block;width:100%;box-sizing:border-box;padding:14px;border:none;border-radius:14px;background:linear-gradient(90deg,#a9812f,#f6dfa0,#caa14d,#f6dfa0,#a9812f);background-size:300% 100%;animation:subGoldShift 5s linear infinite;color:#1c130a;font-weight:800;font-size:14px;letter-spacing:.2px;cursor:pointer;box-shadow:0 6px 20px rgba(246,223,160,.25);transition:transform .15s,box-shadow .15s;}
    .sub-config-btn-primary:active{transform:scale(.98);box-shadow:0 3px 10px rgba(246,223,160,.2);}
    @keyframes subGoldShift{0%{background-position:0% 50%;}100%{background-position:300% 50%;}}
    .sub-config-btn-ghost{display:block;width:100%;box-sizing:border-box;padding:12px;border-radius:14px;background:rgba(246,223,160,.06);border:1px dashed rgba(246,223,160,.5);color:var(--honey,#f6dfa0);font-weight:700;font-size:12.5px;cursor:pointer;transition:.15s;}
    .sub-config-btn-ghost:hover{background:rgba(246,223,160,.12);}
    .sub-config-btn-outline{display:block;width:100%;box-sizing:border-box;padding:12px;border-radius:14px;background:transparent;border:1px solid var(--border,#3a2e40);color:var(--text-muted,#a89bb0);font-weight:600;font-size:12.5px;cursor:pointer;transition:.15s;}
    .sub-config-btn-outline:hover{border-color:var(--honey,#f6dfa0);color:var(--text,#fff);}
    .sub-pack-row{position:relative;border:1px solid rgba(246,223,160,.22)!important;border-radius:16px!important;padding:16px!important;background:linear-gradient(160deg,rgba(246,223,160,.06),rgba(28,20,32,0) 60%);box-shadow:0 4px 16px rgba(0,0,0,.25);}
    .sub-pack-type-toggle{display:inline-flex;background:var(--bg,#120d16);border:1px solid var(--border,#3a2e40);border-radius:12px;padding:3px;gap:2px;}
    .sub-pack-type-btn{border:none;background:none;color:var(--text-muted,#a89bb0);font-size:11.5px;font-weight:700;padding:7px 12px;border-radius:9px;cursor:pointer;display:flex;align-items:center;gap:5px;transition:.15s;}
    .sub-pack-type-btn.active{background:linear-gradient(90deg,var(--honey,#f6dfa0),#e0455a);color:#1c130a;box-shadow:0 2px 8px rgba(246,223,160,.3);}
    .sub-pack-rm-btn{width:28px;height:28px;border-radius:50%;border:1px solid var(--border,#3a2e40);background:var(--bg,#120d16);color:var(--text-muted,#a89bb0);cursor:pointer;font-size:13px;display:flex;align-items:center;justify-content:center;transition:.15s;}
    .sub-pack-rm-btn:hover{border-color:#e0455a;color:#e0455a;}
    .sub-pack-label{display:block;font-size:10.5px;font-weight:700;letter-spacing:.3px;text-transform:uppercase;color:var(--honey,#f6dfa0);margin:0 0 5px;}
    .sub-pack-input{display:block;width:100%;box-sizing:border-box;background:var(--bg,#120d16);border:1px solid var(--border,#3a2e40);color:var(--text,#fff);border-radius:10px;padding:10px 12px;font-size:13px;margin-bottom:12px;transition:.15s;}
    .sub-pack-input:focus{outline:none;border-color:var(--honey,#f6dfa0);box-shadow:0 0 0 3px rgba(246,223,160,.15);}
    .sub-config-desc-input{display:block;width:100%;box-sizing:border-box;background:var(--bg,#120d16);border:1px solid var(--border,#3a2e40);color:var(--text,#fff);border-radius:12px;padding:12px;font-size:12.5px;line-height:1.5;resize:vertical;}
...
async function openSubPackSender(){
  ...
  try{
    ...
    listEl.innerHTML = packs.map((p, idx) => `
      <button type="button" class="coach-table-row" data-idx="${idx}">${typeIconSvg[p.type || 'photo'] || ''} ${(p.type||'photo')} — ${Math.max(1, Math.round(p.price * 0.95 * 100) / 100)}€${p.description ? ' · ' + escText(p.description) : ''}</button>
    `).join('');
...
async function sendSubPack(pack){
  ...
  const ctx = chatCtx;
  // Pack "abonnés" = toujours vendu avec la remise abonné de 5% (c'est le principe même de ce bouton).
  const finalPrice = Math.max(1, Math.round(pack.price * 0.95 * 100) / 100);
  const orderText = t('subPackMessageText').replace('{price}', finalPrice);
  try{
    ...
    await convRef.collection('messages').add({
      ...
      orderKind: 'subPack', tipPrice: finalPrice, tipPriceBeforeDiscount: pack.price,
  ...
  backdrop.innerHTML = `
    <div class="subscribe-page">
      ...
      <div class="subscribe-page-perks">
        <div class="subscribe-perk">${ICON_CAMERA} ${t('subscribePerkPhoto')}</div>
        <div class="subscribe-perk">${ICON_VIDEO} ${t('subscribePerkVideo')}</div>
        <div class="subscribe-perk">${ICON_AUDIO} ${t('subscribePerkAudio')}</div>
        <div class="subscribe-perk sub-discount-note" style="background:transparent;border:none;padding:2px 0 0;">${t('subDiscountNote')}</div>
      </div>
      <button type="button" class="sub-config-btn-primary subscribe-pay-btn" id="subscribe-pay-btn">${t('subscribePayBtn')}</button>
      <p class="subscribe-fictif-note">${t('subscribeFictifNote')}</p>
      <button type="button" class="sub-config-btn-outline" id="subscribe-back-btn">${t('subscribeBackBtn')}</button>
    </div>
  `;
  document.body.appendChild(backdrop);
  document.getElementById('subscribe-back-btn').onclick = () => backdrop.remove();
...
function openSubscriptionConfigPage(m){
  ...
  function rowHtml(pack, idx){
    ...
    return `
      <div class="sub-pack-row" data-idx="${idx}">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-wrap:wrap;gap:6px;">
          <div class="sub-pack-type-toggle">
            <button type="button" class="sub-pack-type-btn ${type==='photo'?'active':''}" data-idx="${idx}" data-type="photo">${ICON_CAMERA} ${t('tipMenuColPhoto')}</button>
            <button type="button" class="sub-pack-type-btn ${type==='video'?'active':''}" data-idx="${idx}" data-type="video">${ICON_VIDEO} ${t('tipMenuColVideo')}</button>
            <button type="button" class="sub-pack-type-btn ${type==='audio'?'active':''}" data-idx="${idx}" data-type="audio">${ICON_AUDIO} ${t('tipMenuColAudio')}</button>
          </div>
          <button type="button" class="sub-pack-rm-btn sub-pack-rm" data-idx="${idx}">✕</button>
        </div>
        <label class="sub-pack-label">${t('subPackPriceLabel')}</label>
        <input type="number" class="sub-pack-input sub-pack-price" data-idx="${idx}" min="10" max="100" step="1" value="${pack.price||''}" placeholder="10–100€">
        <p class="sub-pack-sold-note" id="sub-pack-sold-${idx}">${subPackSoldNoteText(pack.price)}</p>
        <label class="sub-pack-label">${t('subPackDescLabel')}</label>
        <input type="text" class="sub-pack-input sub-pack-desc" data-idx="${idx}" maxlength="60" value="${escAttr(pack.description||'')}" placeholder="${escAttr(t('subPackDescPh'))}">
        <div>
          ${pack.contentUrl ? `
            <div class="tipmenu-content-preview">
  ...
  function render(){
    backdrop.innerHTML = `
      <div class="subscribe-page" style="max-width:520px;text-align:left;">
        ...
        <button type="button" class="sub-config-btn-ghost" id="sub-add-row" style="margin-top:12px;">+ ${t('subConfigAddPack')}</button>
        <a href="https://www.google.com/search?q=euro+to" target="_blank" rel="noopener" class="sub-config-btn-outline" style="display:flex;align-items:center;justify-content:center;gap:6px;margin-top:14px;text-decoration:none;">${ICON_EXTLINK} ${t('tipMenuCurrencyCompareLink')}</a>
        <button type="button" class="sub-config-btn-primary" id="sub-config-save" style="margin-top:14px;">${t('subConfigSaveBtn')}</button>
        <button type="button" class="sub-config-btn-outline" id="sub-config-back" style="margin-top:10px;">${t('subscribeBackBtn')}</button>
      </div>
    `;
    document.getElementById('sub-desc-input').oninput = (e) => { localDesc = e.target.value; };
    backdrop.querySelectorAll('.sub-pack-type-btn').forEach(btn => {
    ...
    backdrop.querySelectorAll('.sub-pack-price').forEach(inp => {
      ...
      const noteEl = document.getElementById('sub-pack-sold-' + idx);
      inp.oninput = () => {
        let val = parseInt(inp.value, 10);
        if(!isNaN(val) && val > 100){ val = 100; inp.value = 100; }
        localPacks[idx].price = isNaN(val) ? null : Math.max(0, val);
        if(noteEl) noteEl.innerHTML = subPackSoldNoteText(localPacks[idx].price);
      };
      inp.onblur = () => {
        const clamped = Math.max(10, Math.min(100, parseInt(inp.value, 10) || 10));
        inp.value = clamped;
        localPacks[idx].price = clamped;
        if(noteEl) noteEl.innerHTML = subPackSoldNoteText(clamped);
      };
    });
